...
from .serializers import (
    SerializadorCronograma,
    SerializadorTarefa,
    SerializadorAluno,
    SerializadorLogin,
    SerializadorCadastro,
)
...
class CronogramaViewSet(viewsets.ModelViewSet):
    ...
    def tarefas_semana(self, request, pk=None):
        ...
        tarefas = Tarefa.objects.filter(
            cronograma=cronograma, data__gte=inicio, data__lte=fim
        ).order_by("data")
        serializer = SerializadorTarefa(tarefas, many=True)
        return Response(serializer.data)
    ...
    def semana1(self, request, pk=None):
        ...
        cronograma = get_object_or_404(Cronograma, pk=self.get_object().pk)
        tarefas = Tarefa.objects.filter(
            cronograma=cronograma, data__gte=semana1, data__lte=fim
        ).order_by("data")
        serializer = SerializadorTarefa(tarefas, many=True)
        return Response(serializer.data)
...
class AlunoViewSet(viewsets.ModelViewSet):
    ...
    def alerta(self, request, pk):
        ...
        tarefas_t = (
            Tarefa.objects.filter(cronograma__aluno=pk)
            .filter(data__gt=now)
            .filter(data__lt=aft)
        )
        print(tarefas_t)
        ...
        if tarefas:
            ...
            try:
                email.send(
                    "Tarefas Pendentes",
                    msgRetorno,
                    ["deividson.silva@escolar.ifrn.edu.br"],
                )
            except:
    ...
    def cadastro(self, request):
        ...
        if user:
            return Response(
                {"error": "Usuário já cadastrado"}, status=status.HTTP_400_BAD_REQUEST
            )
                ...
                return Response(
                    {
                        ...
                        "senha": password + "\n",
                        "confirmação": confirmed_password,
                    },
...
class AuthViewSet(viewsets.GenericViewSet):
    ...
    @action(
        ...
        methods=["post"],
        url_path="login",
        serializer_class=SerializadorLogin,
    )
    def login(self, request):
        ...
        if user is not None:
            ...
            return Response(
                {"detail": "login realizado com sucesso", "user": id},
                status=status.HTTP_200_OK,
            )
        else:
            return Response(
    ...
    @action(
        ...
        methods=["post"],
        url_path="cadastro",
        serializer_class=SerializadorCadastro,
    )
    def cadastro(self, request):
        # pega os dados do formulario
        username = request.data.get("usuario")
        ...
        # verifica se as senhas conferem
            ...
            # cria o usuario
                ...
                request.session["usuario"] = username
                return Response(
                    {"message": "Usuário cadastrado com sucesso", "user": username},
                    status=status.HTTP_200_OK,
                )
